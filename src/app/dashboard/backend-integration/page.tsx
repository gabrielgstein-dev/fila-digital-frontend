...
import { useTokenManagerContext } from '@/components/TokenManagerProvider';
...
export default function IgniterDashboardPage() {
  ...
  const { tokenState, refreshToken, isRefreshing } = useTokenManagerContext();
  ...
  const loadIgniterData = useCallback(async () => {
    ...
    try {
      ...
      const [metricsData, chartDataResult, sessionInfoData] = await Promise.all([
        apiClient.getIgniterPrivateMetrics(),
        apiClient.getIgniterChartData({ type: 'users', period: 'day' }),
